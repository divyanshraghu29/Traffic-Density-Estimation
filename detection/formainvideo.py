...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        lineypos = 95
        cv2.line(image, (400, lineypos), (650, lineypos), (255, 0, 0), 5)

        # line y position created to count contours
        lineypos2 = 450
        cv2.line(image, (170, lineypos2), (750, lineypos2), (0, 255, 0), 5)
        ...
        for i in range(currentcars):  # loops through all current car ids on screen
            ...
            if curcent:  # if there is a current centroid
                ...
                if oldcent:  # checks if old centroid exists
                    ...
                    # to count cars and that car hasn't been counted yet
                    elif oldcent[1] >= lineypos and curcent[1] <= lineypos and carids[
                        currentcarsindex[i]] not in caridscrossed:

                        carscrosseddown = carscrosseddown + 1
                        cv2.line(image, (0, lineypos), (width, lineypos), (0, 0, 125), 5)
                        caridscrossed.append(currentcarsindex[i])

        # Top left hand corner on-screen text
        

        cv2.putText(image, "Cars Entered: " + str(2+carscrossedup), (450, 30), cv2.FONT_HERSHEY_SIMPLEX, .75, (0, 0, 255),
                    2)
        cv2.putText(image, "Cars Entered: " + str(carscrosseddown), (450, 60), cv2.FONT_HERSHEY_SIMPLEX, .75, (0, 0, 255),
                    2)
        totalcars=2+carscrossedup-carscrosseddown
        cv2.putText(image, "Total Cars: " + str(totalcars), (450, 90), cv2.FONT_HERSHEY_SIMPLEX, .75, (0, 0, 255),
                    2)
        ...
        k = cv2.waitKey(int(1)) & 0xff  # int(1000/fps) is normal speed since waitkey is in ms
        if k == 27:
            break
